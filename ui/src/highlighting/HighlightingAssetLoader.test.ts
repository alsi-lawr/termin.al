...
test("returns a typed failure when a known grammar asset cannot load", async () => {
  const requests: Array<string> = [];
  ...
  const loaded = await new HighlightingAssetLoader(failingFetch).load("ql", new AbortController().signal);

  assert.equal(loaded.kind, "failed");
  if (loaded.kind !== "failed") assert.fail("Expected a typed loader failure.");
  assert.equal(loaded.canonical, "CodeQL");
  assert.match(loaded.error.message, /Failed to load highlighting assets for CodeQL/u);
});
